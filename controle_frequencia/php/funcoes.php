...
function update_e_log($s_Tabela, $array_Campo_Registro, $dt_Data,
        $s_CPF_Alterado, $s_CPF_Responsavel, $i_ID)
{
  $s_Campo = '';
  $s_Registro = '';
  $cont = 0;
...
//NOTA: $ts_Entrada RECEBE, POR PADRÃO, "dd/mm/aaaa hh:mm:ss"
//RETORNO NULL = PESQUISA INVÁLIDA
function retorna_id_ponto_funcionario($s_CPF, $ts_Entrada)
{
  $sql = <<<HEREDOC
          SELECT ID_PONTO FROM PONTO_FUNCIONARIO WHERE CPF = '$s_CPF' AND ENTRADA = '$ts_Entrada';
HEREDOC;
  $query = pg_query($sql);
  $result = pg_fetch_result($query, 0, 0);
  if ($result){
